  ...
  <div class="text-xs-center">
    <v-menu offset-y>
      <template v-slot:activator="{ on }">
        <v-btn color="black" dark v-on="on" v-text="curentLanguage" flat> </v-btn>
      </template>
      <v-list>
        <v-list-tile
...
export default {
  ...
  data() {
    return {
      ...
      curlanguage: localStorage.getItem('curentLanguage')
        ? localStorage.getItem('curentLanguage')
        : 'Русский'
    }
  },
  computed: {
  ...
  methods: {
    ...
    SetUpLanguageFromURLParameter() {
      ...
      this.curlanguage = _.find(this.dictionaryLanguage, function(o) {
