          ...
          <v-container pa-0>
            <v-layout>
              <v-flex xs12>
                <v-layout>
                  <v-flex xs6>
                    <v-tabs centered fixed-tabs v-model="menu.activeTab">
                      <v-tab v-for="item in menu.tabs" :key="item">{{ item }} </v-tab>
                    </v-tabs>
                    <v-divider></v-divider>
                    <v-tabs-items v-model="menu.activeTab">
                      <v-tab-item class="white" v-for="item in menu.tabs" :key="item">
                        <v-layout row wrap>
                            ...
                            <v-layout column text-xs-left>
                              ...
                              <v-flex d-flex justify-space-between class="caption grey--text">
                                <span class="text-xs-left"
                                  >Дата добавления: {{ task.dateAdd }}</span
                                >
                                <span class="text-xs-right"
                                  >Плановая дата: {{ task.dateplan }}</span
                                >
                              </v-flex>
                            </v-layout>
                          </v-flex>
...
export default {
  ...
  methods: {
    loadDataForComponents() {
      const datasource = {
        query: `{ smtasks { tasksGetInfo(taskId:${
