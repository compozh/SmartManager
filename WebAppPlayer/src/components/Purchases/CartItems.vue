<template>
  <v-layout column>
    <v-layout row>
      <v-btn block outline round large color="success" :disabled="!cartlist" @click="mutationSubmit"
        >Сформировать заказ</v-btn
      >
      &nbsp;
      <v-btn
        block
          ...
          <v-flex xl9 lg9 md8 sm7 xs6 align-center justify-start resource-cartion>
            <v-tooltip bottom>
              <template v-slot:activator="{ on }">
                <span v-on="on">
                  <h3>
                    <span class="resource-group-text"
                      >[{{
                        cartItem.resource.resourceGroup.name
                          | truncate(getTextLength(20, 20, 40, 40, 40), "...")
                      }}]</span
                    >
                    {{
                      cartItem.resource.name
                        | truncate(getTextLength(100, 80, 200, 500, 500), "...")
                    }}
                  </h3>
                </span>
              </template>
              <span class="hidden-lg-and-up">
                <span>{{ cartItem.resource.name }}</span
                ><br />
                <img
                  class="hidden-sm-and-up"
                  src="https://baxov.net/sites/default/files/2018-03/ima23ge_2.png"
                  width="100px"
                />
              </span>
            </v-tooltip>
