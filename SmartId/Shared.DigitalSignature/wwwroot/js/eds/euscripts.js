...
var EUSignCPHelper = NewClass({
	...
	"DefaultCaSettingsSessionStorageName": "CaSettings",
...
},
{
    initialize: function(params, success, error) {
        ...
        this.CaSettingsStorageName = params.storagePrefix + this.DefaultCaSettingsSessionStorageName;
	    ...
	    try {
		    ...
		    //this.setOcspAccessInfoSettings();
    ...
    //setOcspAccessInfoSettings: function() {
	//    try {
	...
	//		//    settings.SetAddress(this.CAsServers[i].ocspAccessPointAddress);
	//		//    settings.SetPort(this.CAsServers[i].ocspAccessPointPort);

	//		//    for (var j = 0; j < this.CAsServers[i].issuerCNs.length; j++) {
	//		//	    settings.SetIssuerCN(this.CAsServers[i].issuerCNs[j]);
	//		//	    euSign.SetOCSPAccessInfoSettings(settings);
	//		//    }
	//	    //}
	//	    this.setXMLHTTPDirectAccess();
	//    } catch (e) {
	//    }
    //},
    //setXMLHTTPDirectAccess: function () {
	//    return;
	//    try {
	...
	//	    for (var i = 0; i < this.CAsServers.length; i++) {
	//		    if (!this.CAsServers[i].directAccess)
	//			    continue;

	//		    _addDNSName(this.CAsServers[i].address, directAccessURLs);
	//		    _addDNSName(this.CAsServers[i].tspAddress, directAccessURLs);
	//		    _addDNSName(this.CAsServers[i].cmpAddress, directAccessURLs);
	//		    _addDNSName(this.CAsServers[i].ocspAccessPointAddress,
	//			    directAccessURLs);
	//	    }
	...
	setCaFromObj: function(caServer) {
		try {
			var offline = ((caServer == null) ||
					(!caServer.cmpAddress && !caServer.ocspAccessPointAddress && !caServer.tspAddress)) ?
				true : false;
			...
			this.setTspSettings(!offline || (caServer != null && caServer.certsInKey),
				caServer != null && caServer.tspAddress != "" ? caServer.tspAddress : "acskidd.gov.ua",
				caServer != null && caServer.tspAddress != "" ? caServer.tspAddressPort : "80");


			if (useCMP) {
				this.setCmpSettings(true, caServer.cmpAddress);
			} else {
				this.setCmpSettings(false);
			}
			this.setLdapSettings(caServer != null && caServer.ldapAddress,
				caServer != null ? caServer.ldapAddress : null,
				caServer != null ? caServer.ldapAddressPort : null);
			this.caServer = caServer;
		} catch (e) {
			this.onError(e);
		}
	},
	setLdapSettings(useLdap, ldapAddress, ldapPort) {
	...
	setTspSettings(useTsp, tspAddress, tspPort) {
	...
	setOCSPSettings(useOcsp, beforeStore, ocspAddress, ocspPort) {
    ...
    removeStoredPrivateKey: function () {
        ...
        this.removeStorageItem(this.CaSettingsStorageName);
    },
    //-----------------------------------------------------------------------------
    getPrivateKeyCertificatesByCMP: function (key, password, onSuccess, onError, servers) {
    	try {
		    var cmp = [];
            ...
            var certificates = euSign.GetCertificatesByKeyInfo(keyInfo, cmp);
    ...
    getPrivateKeyCertificates: function (key, password, fromCache, onSuccess, onError) {
		...
		if (this.useCMP) {
            this.getPrivateKeyCertificatesByCMP(key, password, onSuccess, onError, [this.caServer]);
    ...
    readPrivateKey: function (keyName, key, password, certificates, fromCache, onSuccess, onError) {
		...
		if (this.getAutoDetect()) {
			var cas = this.CAsServers.slice(0);
			var glSignCa = this.getCaFromGlsign();
			if (glSignCa) {
				cas.splice(0, 0, glSignCa);
			}
			if (fromCache) {
				var jsonSettings = this.getStorageItem(euSignHelper.CaSettingsStorageName, false, false);
				if (jsonSettings) {
					var savedCa = JSON.parse(jsonSettings);
					if (savedCa) {
						cas.splice(0, 0, savedCa);
					}
				}
			}
			...
			var keyInfo;
			try {
				keyInfo = euSign.GetKeyInfoBinary(key, password);
			} catch (e) {
				_onError(e);
				return;
			}
			for (var i = 0; i < cas.length; i++) {
				var ca = cas[i];
				if (ca.cmpAddress) {
					var cmp = ca.cmpPort ? ca.cmpAddress + ":" + ca.cmpPort : ca.cmpAddress;
					try {
						certificates = euSign.GetCertificatesByKeyInfo(keyInfo, [cmp]);
						that.setCaFromObj(ca);
						that.readPrivateKeyBinary(keyName,
							key,
							password,
							certificates,
							true,
        ...
        this.readPrivateKeyBinary(keyName, key,
	        password,
	        certificates,
			true,
	        function() {
    ...
    readPrivateKeyBinary: function (keyName, key, password, certificates, detectCa, success, error) {
	    if (certificates) {
	    ...
	    try {
	        ...
	        if (utils.IsSessionStorageSupported() && euSignHelper.allowStoreKey) {
		        ...
		        } else if (this.CAServer) {
			        this.setStorageItem(euSignHelper.CaSettingsStorageName, JSON.stringify(this.CAServer), false);
		        }
	        }
	        success();
	        return;
