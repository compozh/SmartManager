...
},
{
	...
	SetLocalStorageItem: function (name, item, protect) {
	    try {
	        if (protect) {
	            var protectedData = this.euSign.ProtectDataByPassword(
					item, null, true);
	            window.localStorage.setItem(name, protectedData);
	        } else {
	            if (typeof item === 'string') {
	...
	SetLocalStorageItems: function (name, items, protect) {
	    var length = items.length;

	    if (!this.SetLocalStorageItem(name + 'Count',
				length.toString(), protect)) {
	        return false;
	    }

	    for (var i = 0; i < length; i++) {
	        if (!this.SetLocalStorageItem(name + i.toString(),
					items[i], protect)) {
	            for (var j = 0; j < i; j++) {
	                this.RemoveLocalStorageItem(name + j.toString());
	            }

	            this.RemoveLocalStorageItem(name + 'Count');
	...
	GetLocalStorageItem: function (name, decode, protect) {
	    try {
	...
	GetLocalStorageItems: function (name, decode, protect) {
	    var length = this.GetLocalStorageItem(
			name + 'Count', false, false);
	    if (length == null)
	        return null;
	    ...
	    for (var i = 0; i < length; i++) {
	        var item = this.GetLocalStorageItem(
				name + i.toString(), decode, protect);
	        if (item == null)
	            return false;
	...
	RemoveLocalStorageItems: function (name) {
	    var length = this.GetLocalStorageItem(
			name + 'Count', false, false);
	    if (length == null)
	        return true;
	    ...
	    for (var i = 0; i < length; i++) {
	        this.RemoveLocalStorageItem(name + i.toString());
	    }
