(function () {
	...
	var translates = {
		en: {
			...
			"CouldNotDetectCa": "Не удалось определить центр сертификации, который выдал ключ. Укажите центр сертификации вручную и повторите попытку",
			"PhoneFormat": "Enter phone number in format 380ХХХХХХХХХ",
			"ErrorMessage": "Signature Overlay Error"
		},
		uk: {
			"CA": "ЦСК",
			...
			"KeyInLocalStorageClient": "Ключ буде збережений в браузері для майбутнього використання. Для видалення ключа із браузера необхідно викликати функцію \"Інструменти і налаштування → Електронний підпис → Очистити ключ\"",
			"Sign": "Підписати",
			...
			"Device": "Носій ключа",
			"NoDevices": "Не виявлено доступних носіїв",
			"CouldNotDetectCa": "Не визначити центр сертифікації ключів, який видав ключ. Вкажіть центр сертифікації вручну і повторіть спробу",
            "PhoneFormat": "Введіть номер телефона у форматі 380ХХХХХХХХХ",
            "ErrorMessage": "Виникла помилка підпису."
		},
		ru: {
			"CA": "ЦСК",
			...
			"Device": "Носитель ключа",
			"NoDevices": "Не обнаружено доступных носителей",
			"CouldNotDetectCa": "Не удалось определить центр сертификации, который выдал ключ. Укажите центр сертификации вручную и повторите попытку",
            "PhoneFormat": "Введите номер телефона в формате 380ХХХХХХХХХ",
            "ErrorMessage": "Возникла ошибка подписи"
		}
	}
    ...
    function SendMobileRequest(args, onSuccess, onError) {
        var url = "https://mobileid.it.ua";
        var that = this;
        $.ajax({
            url: url,
            type: "POST",
            ...
            success: function (e) {
                if (!e || !e.Sign) {
                    this.error(that.edsWrapper.getPhase("ErrorMessage"));
                    return;
                };
                onSuccess(e);
                that.edsWrapper.hideDialog();
            },
            error: function (data) {
                onError(that.edsWrapper.getPhase("ErrorMessage"));               
            }
        });
    }
	...
	EdsWrapper.prototype.setParams = function(params) {
		this.Params = params;
		...
		var initProvider = function () {
			return new Promise(function (resolve, reject) {
				...
				if (!that.ProviderName) {
					that.ProviderName = that.GlSign.PreferHarware ? "agent" : "web";
				}
	...
	EdsWrapper.prototype.initProvider = function (name, success, error) {
		...
		var initProps = {
			...
			onDetectCaFailed: function () {
				...
				that.showWarning(that.getPhase("CouldNotDetectCa"));
			},
		...
		if (frame.length > 0) {
			if (this.initializedProviders[name]) {
				...
				frame.get(0).contentWindow.initialize(initProps,
					function() {
						that.postInitialize();
						successFunc();
					}, error);
	...
	EdsWrapper.prototype.createDialog = function() {
		...
		jsFormData.push({ type: "checkbox", label: "MobileID", name: "mobileID", checked: this.ProviderName === "MobileID", position: "label-right" });
		jsFormData.push({
			type: "block",
			name: "devices_block",
			className: "devices-block"
,			list: [
				{ type: "select", label: this.getPhase("Device"), value: "", name: "device", inputWidth: "450" },
				{ type: "newcolumn" },
        ...
        jsFormData.push( { type: "select", label: this.getPhase("Operator"), value: "", name: "operators", options: [
                { text: "Vodafone", value: 1 },
                { text: "Київстар", value: 2 }
            ]});		
        jsFormData.push({ type: "input", label: this.getPhase("Phone"), title: "123", textContent: "380xxxxxxxxx", name: "phone" });
        jsFormData.push({ type: "label", label: this.getPhase("PhoneFormat"), position: "label-right", name: "phoneFormat" });
		...
		this.form.attachEvent("onChange", function (name, value, isChecked) {
			...
			} else if(name === "mobileID"){
                that.Params.action = isChecked ? "signMobile" : that.Params.action;
			    that.changeProvider(isChecked ? name : "web")
            }
		});
		...
		this.form.attachEvent("onButtonClick", function (name) {
			if (name === "btn") {
				if (that.RequestCa) {
					var ca = that.form.getItemValue("ca");
					that.setCa(ca);
				}
	...
	EdsWrapper.prototype.setDialogLabels = function () {
		...
		if (this.Params.actionParams.actionCaption) {
			caption = this.Params.actionParams.actionCaption;
		}
		...
		if (this.dialog) {
			if (this.Params.actionParams.dialogCaption) {
				caption = this.Params.actionParams.dialogCaption;
			}
	...
	EdsWrapper.prototype.setFieldsVisibility = function () {
		...
		if(this.ProviderName === "mobileID"){
            this.form.checkItem("mobileID");
            this.form.hideItem("use_agent");
            this.form.showItem("phone");
            this.form.showItem("phoneFormat");            
            this.form.showItem("operators");
            this.form.hideItem("password");
        } else {
            this.form.hideItem("phone");            
            this.form.hideItem("phoneFormat");            
            this.form.hideItem("operators");            
            this.form.showItem("password");
        }
		if (this.ProviderName === "agent") {
            this.form.hideItem("mobileID");
			this.form.showItem("devices_block");
			this.form.showItem("device");
		} else {
            this.form.showItem("mobileID");
	...
	EdsWrapper.prototype.refreshDevices = function () {
		...
		if (types && types.length) {
			...
			items.push({ text: this.getPhase("NoDevices"), value: "-1" });
		}
	...
	EdsWrapper.prototype.onActionClick = function () {
		...
		var that = this;
        this.Params.action = this.ProviderName == "mobileID" ? "signMobile" : this.Params.action;
		this.form.setItemValue("agent_warning", null);
		switch (this.Params.action) {
			case "signFile":
			case "signData":
			...
			case "getKeyInfo":
				...
				if (this.ProviderName === "web") {
					...
					provider.readHardwarePrivateKey(keyInfo[0],
						...
						[],
						function () {
							that.form.setItemValue('password', null);
							that.performAction();
						});
				}
				break;
			...
			case "verifyData":           
				...
				break;
            case "signMobile":
				that.Params.actionParams.phone = this.form.getItemValue("phone");
				that.Params.actionParams.operator = this.form.getItemValue("operators");
				that.Params.actionParams.message = "Pidtverdzhennja diy";
                that.Params.onError = function(e){
                    that.showWarning(e);
                };
                this.performAction();
                break;
	...
	EdsWrapper.prototype.doAction = function () {
        ...
        this.readPrivateKey(that.form.getItemValue('private_key'),
            ...
            function() {
	            that.form.setItemValue('private_key', null);
	            that.form.setItemValue('password', null);
	            that.form.setItemValue('certificates', null);
                that.performAction();
            }
        );
    }
    ...
    EdsWrapper.prototype.signMobile = function(fileUrl, phone, callbacks, message, positionId, operator){
        if(!this.Params.action){
            this.Params.action = "signMobile";
        }
        this.Params.actionParams = {
            fileUrl: fileUrl,
			phone: phone,
        ...
        if(!callbacks){
            this.Params.onError = function(){
                edsWrapper.showWarning("Ошибка подписи")
            }            
        }
		this.replaceCallbacks(callbacks);
        this.performAction();
	...
	EdsWrapper.prototype.performAction = function () {
		...
		switch (this.Params.action) {
            ...
            case "signMobile":
                loadDataFromServer(that.Params.actionParams.fileUrl,
                    function (fileData) {
                        var phone = that.Params.actionParams.phone;
                        var operator = that.Params.actionParams.operator;
						var message = that.Params.actionParams.message;
						var positionId = that.Params.actionParams.positionId;
						var args = JSON.stringify({ Event: "SignMobile", File: byteArrayToBase64(fileData), Phone: phone, Message: message, PositionId: positionId, Operator: operator});
                        SendMobileRequest(args, that.Params.onSuccess, that.Params.onError);
                    },
                    function () {
                         //TODO: error
                      },
	...
	EdsWrapper.prototype.unload = function() {
		$("frame#web").remove();
		$("frame#agent").remove();
		$("#eds_placeholder").remove();
	}
	...
	GlSign.prototype.Parse = function (value) {
		...
		this.PreferHarware = this.getZoneValue(settingArray, 16);
	}
