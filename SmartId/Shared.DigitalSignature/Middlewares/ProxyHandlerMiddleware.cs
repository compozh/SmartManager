...
namespace DotNetCore2
{
	public class ProxyHandlerMiddleware
	{
		...
		private async Task<byte[]> SafeReadDataStreamAsync(Stream stream)
		{
			...
			while ((count = await streamReader.BaseStream.ReadAsync(buffer, 0, buffer.Length).ConfigureAwait(false)) > 0)
			{
				await memoryStream.WriteAsync(buffer, 0, count).ConfigureAwait(false);
		...
		private async Task<HttpStatusCode> HandleRequest(HttpContext context)
		{
			...
			if (context.Request.Method == "POST")
			{
				...
				requestData = await SafeReadDataStreamAsync(context.Request.Body);
				if (requestData == null)
	...
	public static class ProxyHandlerMiddlewareExtensions
	{
		...
		{
			builder.MapWhen(
				context => context.Request.Path.ToString().EndsWith("/ProxyHandler.ashx"),
				appBranch => {
					// ... optionally add more middleware to this branch
					appBranch.UseMiddleware<ProxyHandlerMiddleware>();
			});
			return builder;
		}
	}
