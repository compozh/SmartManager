    ...
    <div class="vx-row" v-if="cartItems.length">
      ...
      <div :class="classs">
        <div class="items-list-view" v-for="(item, index) in cartItems" :key="item.objectID">
          <item-list-view :min-mode="minMode" :item="item">
            <!-- SLOT: ITEM META -->
            <template slot="item-meta">
              ...
              <p v-if="!minMode" class="mt-4 font-bold text-sm">Quantity</p>
              <vs-input-number v-if="!minMode"
                min="1"
                max="10"
                :value="item.quantity"
            ...
            <!-- SLOT: ACTION BUTTONS -->
            <template v-if="!minMode" slot="action-buttons">
      ...
      <!-- RIGHT COL -->
      <div v-if="!minMode" class="vx-col lg:w-1/3 w-full">
        <vx-card>
...
export default {
  ...
  computed: {
    classs() {
      return 'vx-col w-full relative' + (this.minMode ? '' : ' lg:w-2/3 ')
    },
    cartItems() {
