<template>
    <v-container>
        ...
        <v-text-field
          ...
          required 
        />
        <v-btn outlined @click="createFormio" color="#326DA8">Создать форму</v-btn>
        <v-btn outlined @click="saveFormio" color="#326DA8">Сохранить форму</v-btn>
    </v-container>
</template>
...
export default {
  name: 'mes-quality',
  components: { formbuilder: Formio, apexchart: VueApexCharts },
  data() {
      return {formCode: ''}
  },
  methods: {
      createFormio() {
        this.$store.dispatch('mes/createFormio', { params : this.formioParams(), callback: ()=> {} });
      },
      saveFormio() {
          var params = this.formioParams();
          params.name = "Saved form";
        this.$store.dispatch('mes/saveFormio', { params, callback: ()=> {} })
