  ...
  methods: {
    async login() {
      if (!this.userData.login) {
        return (this.message = 'Логин не может быть пустой')
      }
      if (!this.userData.password) {
        return (this.message = 'Пароль не может быть пустой')
      }
      this.loading = true
      try {
        const result = await this.$store.dispatch('eam/auth/login', this.userData)
        this.loading = false
