<template>
  <div id="todo-app"
    ...
    <div :class="{'sidebar-spacer': clickNotClose}"
      ...
      <div
        ...
        <feather-icon class="md:inline-flex lg:hidden ml-4 mr-4 cursor-pointer" icon="MenuIcon"
                      @click.stop="toggleTodoSidebar(true)"></feather-icon>

        <!-- SEARCH BAR -->
        <vs-input size="large" icon-pack="feather" icon="icon-search" placeholder="Search..."
                  v-model="searchQuery"
      ...
      <VuePerfectScrollbar class="todo-content-scroll-area" :settings="settings" ref="todoListPS">
        <transition-group class="todo-list" name="list-enter-up" tag="ul" appear>
          <li class="cursor-pointer todo_todo-item" v-for="(todoItem, index) in todoList"
              :key="String(todoFilter) + String(todoItem.id)"
              :style="[{transitionDelay: (index * 0.1) + 's'}]">
            <todo-item :todoItemId="todoItem.id"
                       @showDisplayPrompt="showDisplayPrompt($event)"></todo-item>
          </li>
        </transition-group>
      </VuePerfectScrollbar>
      <!-- /TODO LIST -->
    ...
    <!-- EDIT TODO DIALOG -->
    <todo-edit :displayPrompt="displayPrompt" :todoItemId="todoIdToEdit"
               @hideDisplayPrompt="hidePrompt" v-if="displayPrompt"></todo-edit>
  </div>
</template>
