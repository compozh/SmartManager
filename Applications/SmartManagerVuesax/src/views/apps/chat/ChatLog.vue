  ...
  computed: {
    ...
    senderImg() {
      return (isSentByActiveUser) => {
        if (isSentByActiveUser) { return require(`@/assets/images/portrait/small/${this.$store.state.AppActiveUser.img}`) } else { return require(`@/assets/images/portrait/small/${this.contacts[this.contactIndex].img}`) }
      }
    },
    hasSentPreviousMsg() {
