        ...
        >
          <template v-slot:activator="{on}">
            <v-layout class="user-panel" v-on="on" align-center>
              <v-flex id="user-icon">
                <user-icon :src="user.photo" size="30" style="max-width:30px"></user-icon>
              </v-flex>
            ...
            <v-flex>
              <v-layout pa-2>
                ...
                <v-flex ml-3 class="text-xs-left">
                  <p v-if="mini || $router.breakpoint.smAndDown" class="mb-1">{{ user.login || user.name}}</p>
                    <a v-on:click="params.changePassword"
                       @click="menu = !menu"
                    >Сменить пароль</a>
                </v-flex>
              </v-layout>
